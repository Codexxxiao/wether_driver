...
要求生成 3 个版本的【极短】视频脚本，必须严格控制总字数在 70-80 字以内（口播时长控制在 8-9 秒）：
版本A（恐吓拉扯型）：放大不戴口罩的严重后果（2句话）。
版本B（场景代入型）：设定具体生活场景引发共鸣（2句话）。
版本C（算账省钱型）：对比防晒霜/医美的成本（2句话）。
...
注意：这三部分合并起来的总字数绝对不能超过 80 个字！除了 JSON 数组，不要输出任何其他废话。
`;
